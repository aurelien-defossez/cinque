...
function MetaClass:__tostring()
	return "[x="..(self.x and self.x or "nil").." ; y="..(self.y and self.y or "nil").."]"
end
...
function Class:angle()
	return atan2(self.x, self.y) * 180 / PI
end

-- Return the angle between this vector and another
function Class:vectorAngle(vector)
	return acos(self:dot(vector) / (self:length() * vector:length())) * 180 / PI
end
...
--
-- Usage: vec2():getIntersection(p, q, r, s)
