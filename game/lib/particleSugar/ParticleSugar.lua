...
local Emitter = require("lib.particleSugar.Emitter")
local ParticleType = require("lib.particleSugar.ParticleType")
...
----------------------------------------------------------------------------
singleton 'lib.particleSugar.ParticleSugar'
...
function method:setEmitterTarget(emitterName, dispObj, autoRotate, rotationOffset, xOffset, yOffset)
    ...
    if not em then
        print('emitter not exist', emName, ptName)
        error('emitter not exist')
    end
...
function method:_newParticle(link)
    ...
    local howMany = floor(link.emissionRate * ctx.deltaT * 0.001 * link._skipCnt)
...
function method:_moveParticle(particle)
    ...
    if (particle._fadeOutTime > curT and particle.alpha < 1) then
        ...
        particle.alpha = min(1, particle.alpha + prop.fadeInSpeed * 0.001 * self.deltaT)
    end
